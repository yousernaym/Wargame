Shader "Unlit/WaterShader"
{
    Properties
    {
		_Ground("Ground", 2D) = "green" {}
		[NoScaleOffset]_GroundNormalMap("SeabedNormalMap", 2D) = "black" {}
		...
		_Seabed("Seabed", 2D) = "white" {}
		[NoScaleOffset]_SeabedNormalMap("SeabedNormalMap", 2D) = "black" {}
		[Color]_SeabedTint("Seabed tint", color) = (1, 1, 1, 1)
		...
		_SeabedParallax("Seabed parallax", range(0, 1)) = 0.9
		[Space]
		...
		[Toggle]_EnableSeabed("Enable Seabed", float) = 1
		[Space]
	...
	SubShader
	{
		...
		Pass
		{
            ...
            struct VertexOutput
            {
				...
				float2 seabedUV : TEXCOORD4;
				float2 causticsUV : TEXCOORD5;
				float2 groundUV : TEXCOORD6;
				float4 color : COLOR0;
            };
			...
			// Calculates factor used to blend between seabed and sky depending on difference between viewing angle and water surface angle
			...
			float4 sampleTexture(float2 uv, float2 uvOffset, sampler2D tex, sampler2D nmap, float4 tint, float3 dirToSun)
			{
				uv = uv + uvOffset;
				float4 color = tex2D(tex, uv);
				float3 normal = tex2D(nmap, uv).xyz;
				if (any(normal)) //If there is a normalmap
					normal = normal * 2 - 1;
				else
					normal = float3(0, 0, 1); //Use default normal pointing upwards
				float sunlight = saturate(dot(dirToSun, normalize(normal)) + 0.3f); //Diffuse + ambient lighting
				return color * sunlight * tint;
			}
            ...
            VertexOutput vert(VertexInput input)
            {
				...
				output.seabedUV = (output.worldPos - _WorldSpaceCameraPos * _SeabedParallax) * _Seabed_ST.xy + _Seabed_ST.zw;
				output.causticsUV = (output.worldPos - _WorldSpaceCameraPos * _SeabedParallax) * _Caustics_ST.xy + _Caustics_ST.zw;

				output.groundUV = output.worldPos * _Ground_ST.xy + _Ground_ST.zw;
			...
			fixed4 frag(VertexOutput input) : SV_Target
			{
				...
				if (_EnableSeabed == 1) //"Enable seabed" checkbox is checked in inspector
				{
					float2 seabedUV = input.seabedUV + uvOffset;
					float4 seabedCol = tex2D(_Seabed, seabedUV);
					float3 seabedNormal = tex2D(_SeabedNormalMap, seabedUV).xyz;
					if (any(seabedNormal)) //If there is a normalmap
						seabedNormal = seabedNormal * 2 - 1;
					else
						seabedNormal = float3(0, 0, 1); //Use default normal pointing upwards
					float sunlight = saturate(dot(dirToSun, normalize(seabedNormal)) + 0.3f); //Diffuse + ambient lighting
					seabedCol *= sunlight * _SeabedTint;
					...
					if (_CausticsBrightness > 0)
					{
						...
						//Apply final caustics color to seabed
						seabedCol = saturate(seabedCol + causticsCol);
					}
					//Blend between seabed and water color
					result = lerp(seabedCol, waterCol, _WaterOpacity);
				}
				else
					result = waterCol;
